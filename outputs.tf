...
output "lb_public_url" {
  value = format("curl -v -I --resolve www.example.com:80:%s http://www.example.com/", lookup(oci_load_balancer_load_balancer.oci_kind_lb.ip_address_details[0], "ip_address"))
}
...
output "oci_kind_basic_source_code" {
  value = "https://github.com/marcelo-ochoa/oci-kind-cluster"
}
