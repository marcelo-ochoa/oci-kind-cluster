...
data "oci_database_autonomous_database_wallet" "autonomous_database_wallet" {
  autonomous_database_id = oci_database_autonomous_database.oci_kind_autonomous_database.id
...
data "template_file" "cloud_init" {
  ...
  vars = {
    ...
    kind_multinode_yaml_content     = base64gzip(data.local_file.kind_multinode_yaml.content)
    ...
    db_name                        = oci_database_autonomous_database.oci_kind_autonomous_database.db_name
    assets_url                     = var.object_storage_oci_kind_media_visibility == "Private" ? "" : "https://objectstorage.${var.region}.oraclecloud.com/n/${oci_objectstorage_bucket.oci_kind_media.namespace}/b/${oci_objectstorage_bucket.oci_kind_media.name}/o/"
  }
}
...
data "template_file" "deploy_template" {
  ...
  vars = {
    ...
    oci_kind_media_visibility = var.object_storage_oci_kind_media_visibility
    wallet_par              = "https://objectstorage.${var.region}.oraclecloud.com${oci_objectstorage_preauthrequest.oci_kind_wallet_preauth.access_uri}"
  }
}
...
data "local_file" "kind_multinode_yaml" {
  filename = "${path.module}/scripts/kind-multinode.yaml"
}
locals {
  catalogue_port = 3005
}
...
locals {
  common_tags = {
    Reference = "Created by OCI QuickStart for OciKind Basic demo"
  }
}
